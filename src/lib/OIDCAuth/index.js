...
/**
 * Obtain OIDC access token and periodically refresh it
 */
class OIDCAuth extends EventEmitter {
    /**
     *
     * @param {Object} opts
     ...
     * @param {String} [opts.tokenEndpoint] OIDC token Endpoint URL
     * @param {String} [opts.refreshSeconds] OIDC token refresh interval in seconds
    ...
    constructor(opts) {
        ...
        if ((typeof this._refreshSeconds !== 'number') || (this._refreshSeconds <= 0)) {
            throw new Error('OIDC auth config: refreshSeconds must be a positive integer value');
        }
        if ((typeof this._refreshRetrySeconds !== 'number') || (this._refreshRetrySeconds <= 0)) {
            throw new Error('OIDC auth config: refreshRetrySeconds must be a positive integer value');
        }
        if (!this._logger) {
            throw new Error('OIDC auth config requires logger property');
        }
        ...
        if (opts.tokenEndpoint && opts.clientKey && opts.clientSecret) {
            ...
            this._logger.isDebugEnabled && this._logger.debug('OIDC auth config token API data not set, fallback to static token');
            this._token = opts.staticToken;
        } else {
            this._logger.warn('OIDC auth error: neither token API data nor static token is set');
            this._token = null;
        }
    }
    ...
    async refreshToken() {
        ...
        this._logger.isDebugEnabled && this._logger.debug('OIDC token refresh initiated');
        const reqOpts = {
            ...this._reqOpts,
            headers: {
        ...
        try {
            ...
            this._logger.verbose('Response received from OIDC provider');
